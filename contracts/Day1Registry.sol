...
contract Day1Registry {
  ...
  // event fired on submission of a Day1User entry.
  event registeredDay1UserEvent (
    string _name,
    string _surname,
  ...
  function registerUser(string calldata _name, string calldata _surname) external returns(uint){
    ...
    // trigger event for Day1User registration
    emit registeredDay1UserEvent(_name, _surname, submissionBlockNumber);
